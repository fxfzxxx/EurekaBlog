...
public class RegExp {
    public static void main(String[] args) {
        String content = "He eats two papples";
        checkApple(content);
    }

    public static void checkApple(String content){
        String pattern = ".*apple.*"; //去掉.* 输出为 False	
...
```java
Dose the String contain apple? true
```

## 捕获组(Capture Group)

捕获组是把多个字符当**一个单独单元**进行处理的方法，它通过对括号内的字符分组来创建。
...
```java
Dose the String contain apple? true
Found value: He eats 2 apples and 3 pears
Found value: He eats
Found value: 2
Found value:  apples and 3 pears
```
